...
{
    int tidx = threadIdx.x;
    int channels_per_block = DIV_RUP(c, blockDim.x);

    int blockIdx_channel = blockIdx.x % channels_per_block;
    int blockIdx_spatial = blockIdx.y;

    int threadIdx_channel = tidx % channels_per_block;

    int c_iter = blockIdx_channel * channels_per_block + threadIdx_channel;
    int row_col_idx = blockIdx_spatial * blockDim.x + tidx;

    // Calculate the spatial row and column index
    int row = row_col_idx / i_w;
    int col = row_col_idx % i_h;

    // if thread is accessing out-of-bounds, return
    if (c_iter >= c || row < 0 || row >= i_h || col < 0 || col >= i_w)
    {
        return;
    }
...
__global__ void kernel_max_pooling_smem(FLOATTYPE *in, FLOATTYPE *out, int c, 
                                    int i_h, int i_w, int input_spatial_size,
                                    int f_w, int f_h, int o_h, int o_w, int output_spatial_size)
{
    int tidx = threadIdx.x;
...
{   
    ...
    int blocksPerChannel = DIV_RUP(c, threadsPerBlock);
    int blocksPerSpatial = DIV_RUP(i_h * i_w, threadsPerBlock);
    dim3 blockDim(threadsPerBlock);
    dim3 gridDim(blocksPerChannel, blocksPerSpatial);
    ...
    // kernel_max_pooling_smem<<<c, input_spatial_size, shmem_size>>>(gImage, gOutImage0, c, i_h, i_w, input_spatial_size, 
    //                                             fw0, fh0, o_h0, o_w0, output_spatial_size0);
    
    // kernel_max_pooling_smem<<<c, input_spatial_size, shmem_size>>>(gImage, gOutImage1, c, i_h, i_w, input_spatial_size, 
    //                                             fw1, fh1, o_h0, o_w0, output_spatial_size0);

    // kernel_max_pooling_smem<<<c, input_spatial_size, shmem_size>>>(gImage, gOutImage2, c, i_h, i_w, input_spatial_size, 
    //                                             fw2, fh2, o_h0, o_w0, output_spatial_size0);

    integrated_kernel_max_pooling<<<c, input_spatial_size, shmem_size>>>(gImage, gOutImage0, gOutImage1, gOutImage2, c, i_h, i_w, input_spatial_size, 
...
{   
    ...
    int shmem_size = sizeof(FLOATTYPE) * input_spatial_size;
    kernel_max_pooling_smem<<<c, input_spatial_size, shmem_size>>>(gImage, gOutImage, c, i_h, i_w, input_spatial_size, 
                                                           fw, fh, o_h, o_w, output_spatial_size);
